...
start_m5_50 <- subset(start_m5, V1<=50 & V1 >= -100 & V2 < 41 & V2 >=20);
start_m3_50 <- subset(start_m3, V1<=100 & V1 >= -50 & V2 < 41 & V2 >=20);
...
mLen5<-ggplot(subset(start_m5_50, V1 <= 50 & V1 >= -100 & V2 >=20) , aes(x=V1, y=V2, fill=V3)) + geom_tile()  +
               ...
               scale_fill_gradientn(colours=c("yellow","lightblue","blue", "navy"), values=c(0,15,30,45,60,200), rescaler = function(x,...) x, oob = identity, name="sum count") +
...
ggsave(file=,args[5], out1, width=500, height=150, unit="mm", dpi=300) 
