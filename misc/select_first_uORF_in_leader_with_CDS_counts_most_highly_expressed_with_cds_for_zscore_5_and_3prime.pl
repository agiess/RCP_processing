...
for my $gene (keys %gene_model_fwd){
    ...
    for my $coord (sort {$a <=> $b} keys %{ $gene_model_fwd{$gene} } ){
        ...
        if (($coord >= ($five_prime_coord)) && ($coord <= ($start_coord-1))){  
            my @TIS;
            for(-2 .. 0){
            ...
            #unless ( ($seq_TIS =~ /AT\w/) || ($seq_TIS =~ /A\wG/) || ($seq_TIS =~ /\wTG/) ){ #non near/canoical
            if ( $seq_TIS eq $UORF_START_CODON ){
...
for my $gene (keys %gene_model_rev){
    ...
    for my $coord (sort {$a <=> $b} keys %{ $gene_model_rev{$gene} } ){
        if (($coord >= ($five_prime_coord)) && ($coord <= ($start_coord-1))){  #(+2) at least 30nt from the TSS and TIS
            my @TIS;
            for(-2 .. 0){
...
my %uORF_SSU_coverage_by_positon;
my %uORF_LSU_coverage_by_positon;
my %uORF_window_search_fwd;
my %uORF_window_search_rev;
my %gene2uorf;

my %uORF_SSU_3prime_counts_by_positon;
my %uORF_LSU_3prime_counts_by_positon;
my %uORF_SSU_5prime_counts_by_positon;
my %uORF_LSU_5prime_counts_by_positon;

#my %transcript_SSU_coverage_by_positon; #moved up to transcript assignments
#my %transcript_LSU_coverage_by_positon;
#my %transcript_window_search_fwd;
#my %transcript_window_search_rev;
...
for my $gene (keys %gene_model_fwd){
    ...
    my $gene_three_prime_coord=$three_prime_most_coord_fwd{$gene};

    my $leader_length=$gene_start_coord-$gene_five_prime_coord;
    if ($leader_length >= 200){
 
        if (exists ($gene_to_uORF{$gene})){ 

            for my $uORF_id (sort keys %{ $gene_to_uORF{$gene} }){
                if (exists ($uORF_start_coord{$gene}{$uORF_id})){
                    if ( ($uORF_start_coord{$gene}{$uORF_id} > ($gene_five_prime_coord+100)) && ($uORF_start_coord{$gene}{$uORF_id} < ($gene_start_coord-100) ) ){

                        my $uORF_start_coord=$uORF_start_coord{$gene}{$uORF_id};
                        $gene2uorf{$gene}=$uORF_id; #required for stop codon
                        my $relational_position=-100; #1 to 100 window upstream and downsteam
                        while ($relational_position < 100){
                            my $genomic_position=$gene_model_fwd{$gene}{($uORF_start_coord+$relational_position)};
                            $uORF_window_search_fwd{$chr}{$genomic_position}=$gene."~".$relational_position;
                            $uORF_SSU_coverage_by_positon{$gene}{$relational_position}=0; #initialise
                            $uORF_LSU_coverage_by_positon{$gene}{$relational_position}=0; #initialise
                            $uORF_SSU_3prime_counts_by_positon{$gene}{$relational_position}=0; #initialise
                            $uORF_LSU_3prime_counts_by_positon{$gene}{$relational_position}=0; #initialise
                            $uORF_SSU_5prime_counts_by_positon{$gene}{$relational_position}=0; #initialise
                            $uORF_LSU_5prime_counts_by_positon{$gene}{$relational_position}=0; #initialise
                            $relational_position++;
                        }
                        last; #take the first uORF (closest to the TSS)
                    }
                }        
            }
#            #also set up transcript coverage by position for the calculation of SD and mean for zscore normalisation
#            my $relational_position=0;
#            for my $coord ($gene_five_prime_coord .. $gene_three_prime_coord){
#                my $genomic_position=$gene_model_fwd{$gene}{$coord};
#                $transcript_window_search_fwd{$chr}{$genomic_position}=$gene."~".$relational_position;
#                $transcript_SSU_coverage_by_positon{$gene}{$relational_position}=0; #initialise                           
#                $transcript_LSU_coverage_by_positon{$gene}{$relational_position}=0; #initialise
#                $relational_position++;
#            }
...
for my $gene (keys %gene_model_rev){
    ...
    my $gene_three_prime_coord=$three_prime_most_coord_rev{$gene};
            ...
            for my $uORF_id (sort keys %{ $gene_to_uORF{$gene} }){
                if (exists ($uORF_start_coord{$gene}{$uORF_id})){
                    if ( ($uORF_start_coord{$gene}{$uORF_id} > ($gene_five_prime_coord+100)) && ($uORF_start_coord{$gene}{$uORF_id} < ($gene_start_coord-100) ) ){
                        ...
                        my $relational_position=-100; #1 to 100 window upstream and downsteam
                        while ($relational_position < 100){
                            my $genomic_position=$gene_model_rev{$gene}{($uORF_start_coord+$relational_position)};
                            $uORF_window_search_rev{$chr}{$genomic_position}=$gene."~".$relational_position;
                            $uORF_SSU_coverage_by_positon{$gene}{$relational_position}=0; #initialise
                            $uORF_LSU_coverage_by_positon{$gene}{$relational_position}=0; #initialise
                            $uORF_SSU_3prime_counts_by_positon{$gene}{$relational_position}=0; #initialise
                            $uORF_LSU_3prime_counts_by_positon{$gene}{$relational_position}=0; #initialise
...
#            my $relational_position=0;
#            for my $coord ($gene_five_prime_coord .. $gene_three_prime_coord){
#                my $genomic_position=$gene_model_rev{$gene}{$coord};
#                $transcript_window_search_rev{$chr}{$genomic_position}=$gene."~".$relational_position;
#                $transcript_SSU_coverage_by_positon{$gene}{$relational_position}=0; #initialise                           
#                $transcript_LSU_coverage_by_positon{$gene}{$relational_position}=0; #initialise
#                $relational_position++;
#            }
    ...
    unless ($flag & 0x4){   #if aligned
        ...
        if ($mapq >= 10){     #mapping uniqnes filter
            ...
            if ($flag & 0x10){  #Reverse reads. Starting from the leftmost position parse the cigar and check if it matches a uORF
                ...
                if (exists ($uORF_window_search_rev{$chr}{$threePrime})){
                     my ($gene,$meta_pos)=split("~", $uORF_window_search_rev{$chr}{$threePrime});
                     $uORF_SSU_3prime_counts_by_positon{$gene}{$meta_pos}++;
                }
               ...
               if (exists ($uORF_window_search_rev{$chr}{$fivePrime})){
                     my ($gene,$meta_pos)=split("~", $uORF_window_search_rev{$chr}{$fivePrime});
                     $uORF_SSU_5prime_counts_by_positon{$gene}{$meta_pos}++;
                }
                    ...
                    if ($cigar =~ /^([0-9]+[MIDN])/){
                        ...
                        if ($cigar_part =~ /(\d+)M/){   #alignment matching 
                            for my $pos ($leftMost .. ($leftMost+$1-1)){ #search though this position
                                ...
                                if (exists ($uORF_window_search_rev{$chr}{$pos})){
                                    my ($gene,$meta_pos)=split("~", $uORF_window_search_rev{$chr}{$pos});
                                    $uORF_SSU_coverage_by_positon{$gene}{$meta_pos}++;
                                }
                ...
                if (exists ($uORF_window_search_fwd{$chr}{$threePrime})){
                     my ($gene,$meta_pos)=split("~", $uORF_window_search_fwd{$chr}{$threePrime});
                     $uORF_SSU_3prime_counts_by_positon{$gene}{$meta_pos}++;
                }
                ...
                if (exists ($uORF_window_search_fwd{$chr}{$fivePrime})){
                     my ($gene,$meta_pos)=split("~", $uORF_window_search_fwd{$chr}{$fivePrime});
                     $uORF_SSU_5prime_counts_by_positon{$gene}{$meta_pos}++;
                }
                    ...
                    if ($cigar =~ /^([0-9]+[MIDN])/){
                        ...
                        if ($cigar_part =~ /(\d+)M/){   #alignment matching 
                            for my $pos ($leftMost .. ($leftMost+$1-1)){ #search though this position
                                ...
                                if (exists ($uORF_window_search_fwd{$chr}{$pos})){
                                    my ($gene,$meta_pos)=split("~", $uORF_window_search_fwd{$chr}{$pos});
                                    $uORF_SSU_coverage_by_positon{$gene}{$meta_pos}++;
                                }
...
#for my $u (keys %SSU_coverage_by_positon){
#    for my $f (keys %{ $SSU_coverage_by_positon{$u} }){
#        print "$u,$f\n";
#        for my $p (sort keys %{ $SSU_coverage_by_positon{$u}{$f} }){
#            print "$u,$f,$p,$SSU_coverage_by_positon{$u}{$f}{$p}\n";
#        }
#    }
#}
    ...
    unless ($flag & 0x4){   #if aligned
        ...
        if ($mapq >= 10){     #mapping uniqnes filter
            ...
            if ($flag & 0x10){  #Reverse reads. Starting from the leftmost position parse the cigar and check if it matches a uORF
                ...
                if (exists ($uORF_window_search_rev{$chr}{$threePrime})){
                     my ($gene,$meta_pos)=split("~", $uORF_window_search_rev{$chr}{$threePrime});
                     $uORF_LSU_3prime_counts_by_positon{$gene}{$meta_pos}++;
                }
                ...
                if (exists ($uORF_window_search_rev{$chr}{$fivePrime})){
                     my ($gene,$meta_pos)=split("~", $uORF_window_search_rev{$chr}{$fivePrime});
                     $uORF_LSU_5prime_counts_by_positon{$gene}{$meta_pos}++;
                }
                    ...
                    if ($cigar =~ /^([0-9]+[MIDN])/){
                        ...
                        if ($cigar_part =~ /(\d+)M/){   #alignment matching 
                            for my $pos ($leftMost .. ($leftMost+$1-1)){ #search though this position
                                ...
                                if (exists ($uORF_window_search_rev{$chr}{$pos})){
                                    my ($gene,$meta_pos)=split("~", $uORF_window_search_rev{$chr}{$pos});
                                    $uORF_LSU_coverage_by_positon{$gene}{$meta_pos}++;
                                }
...
my $out_file1 = $outdir."/uORF_coverage.csv";
my $out_file2 = $outdir."/transcript_coverage.csv";

my $out_file3 = $outdir."/uORF_3prime.csv";
my $out_file4 = $outdir."/transcript_3prime.csv";

my $out_file5 = $outdir."/uORF_5prime.csv";
my $out_file6 = $outdir."/transcript_5prime.csv";

#¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤#
# Positional windows for uORFS #
#¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤#
open (OUT1,">$out_file1")  || die "can't open $out_file1\n";
print OUT1 "gene_id,distance_uorf_stop_to_cds,transcript_SSU_count,transcript_LSU_count,cds_SSU_count,cds_LSU_count,cds_SSU_FPKM,cds_LSU_FPKM,transcript_length,cds_length,uorf_kozak_score,gene_overlaps_another_gene,leader_overlap_an_upstream_gene,trailer_overlaps_a_downstream_gene,gene_overlaps_ncRNA,uorf_stop_codon,fraction,position_in_region,count\n";

for my $gene (keys %uORF_SSU_coverage_by_positon){

    my $uorf_id=$gene2uorf{$gene}; #this only works becuase I am selecting genes with 1 ATG uORF
    my $uorf_stop_codon=$uORF_stop_codon{$uorf_id};
    ...
    my $distance_uorf_stop_to_cds=0;
    if (exists ( $start_coord_fwd{$gene})){
        $distance_uorf_stop_to_cds=$start_coord_fwd{$gene}-($uORF_stop_coord{$gene}{$uorf_id}+1);
    }elsif (exists ( $start_coord_rev{$gene})){
        $distance_uorf_stop_to_cds=$start_coord_rev{$gene}-($uORF_stop_coord{$gene}{$uorf_id}+1);
    }
 
    my $kozak_score=0; if (exists ($uORF_kozak{$uorf_id})){ $kozak_score=$uORF_kozak{$uorf_id}; }
    ...
    my $gene_info="$gene,$distance_uorf_stop_to_cds,$transcript_counts_SSU{$gene},$transcript_counts_LSU{$gene},$CDS_counts_SSU{$gene},$CDS_counts_LSU{$gene},$CDS_TCPseq_SSU_FPKM,$CDS_TCPseq_LSU_FPKM,$transcript_length,$cds_length,$kozak_score,$gene_overlaps_another_gene,$leader_overlap_an_upstream_gene,$trailer_overlaps_a_downstream_gene,$gene_overlaps_ncRNA,$uorf_stop_codon";

    #SSU
    for my $pos (sort {$a <=> $b} keys %{$uORF_SSU_coverage_by_positon{$gene}}){ 
        print OUT1 "$gene_info,SSU,$pos,$uORF_SSU_coverage_by_positon{$gene}{$pos}\n"; 
    }

    #LSU
    for my $pos (sort {$a <=> $b} keys %{$uORF_LSU_coverage_by_positon{$gene}}){
        print OUT1 "$gene_info,LSU,$pos,$uORF_LSU_coverage_by_positon{$gene}{$pos}\n"; 
    }
}
close(OUT1);
...
print OUT2 "gene_id,fraction,position_in_region,count\n";

for my $gene (keys %transcript_SSU_coverage_by_positon){

    #SSU
    for my $pos (sort {$a <=> $b} keys %{$transcript_SSU_coverage_by_positon{$gene}}){
         print OUT2 "$gene,SSU,$pos,$transcript_SSU_coverage_by_positon{$gene}{$pos}\n";
    }
    
    #LSU
    for my $pos (sort {$a <=> $b} keys %{$transcript_LSU_coverage_by_positon{$gene}}){
         print OUT2 "$gene,LSU,$pos,$transcript_LSU_coverage_by_positon{$gene}{$pos}\n";
    }
}
close(OUT2);
...
print OUT3 "gene_id,distance_uorf_stop_to_cds,transcript_SSU_count,transcript_LSU_count,cds_SSU_count,cds_LSU_count,cds_SSU_FPKM,cds_LSU_FPKM,transcript_length,cds_length,uorf_kozak_score,gene_overlaps_another_gene,leader_overlap_an_upstream_gene,trailer_overlaps_a_downstream_gene,gene_overlaps_ncRNA,uorf_stop_codon,fraction,position_in_region,count\n";

for my $gene (keys %uORF_SSU_3prime_counts_by_positon){
    ...
    #SSU
    for my $pos (sort {$a <=> $b} keys %{$uORF_SSU_3prime_counts_by_positon{$gene}}){
        print OUT3 "$gene_info,SSU,$pos,$uORF_SSU_3prime_counts_by_positon{$gene}{$pos}\n";
    }

    #LSU
    for my $pos (sort {$a <=> $b} keys %{$uORF_LSU_3prime_counts_by_positon{$gene}}){
        print OUT3 "$gene_info,LSU,$pos,$uORF_LSU_3prime_counts_by_positon{$gene}{$pos}\n";
    }
}
close(OUT3);
...
for my $gene (keys %transcript_SSU_3prime_counts_by_positon){
    ...
    for my $pos (sort {$a <=> $b} keys %{$transcript_SSU_3prime_counts_by_positon{$gene}}){
         print OUT4 "$gene,SSU,$pos,$transcript_SSU_3prime_counts_by_positon{$gene}{$pos}\n";
    }

    #LSU
    for my $pos (sort {$a <=> $b} keys %{$transcript_LSU_3prime_counts_by_positon{$gene}}){
         print OUT4 "$gene,LSU,$pos,$transcript_LSU_3prime_counts_by_positon{$gene}{$pos}\n";
    }
}
close(OUT4);
...
print OUT5 "gene_id,distance_uorf_stop_to_cds,transcript_SSU_count,transcript_LSU_count,cds_SSU_count,cds_LSU_count,cds_SSU_FPKM,cds_LSU_FPKM,transcript_length,cds_length,uorf_kozak_score,gene_overlaps_another_gene,leader_overlap_an_upstream_gene,trailer_overlaps_a_downstream_gene,gene_overlaps_ncRNA,uorf_stop_codon,fraction,position_in_region,count\n";

for my $gene (keys %uORF_SSU_5prime_counts_by_positon){
    ...
    #SSU
    for my $pos (sort {$a <=> $b} keys %{$uORF_SSU_5prime_counts_by_positon{$gene}}){
        print OUT5 "$gene_info,SSU,$pos,$uORF_SSU_5prime_counts_by_positon{$gene}{$pos}\n";
    }

    #LSU
    for my $pos (sort {$a <=> $b} keys %{$uORF_LSU_5prime_counts_by_positon{$gene}}){
        print OUT5 "$gene_info,LSU,$pos,$uORF_LSU_5prime_counts_by_positon{$gene}{$pos}\n";
    }
}
close(OUT5);
...
print OUT6 "gene_id,fraction,position_in_region,count\n";
...
for my $gene (keys %transcript_SSU_5prime_counts_by_positon){
    ...
    for my $pos (sort {$a <=> $b} keys %{$transcript_SSU_5prime_counts_by_positon{$gene}}){
         print OUT6 "$gene,SSU,$pos,$transcript_SSU_5prime_counts_by_positon{$gene}{$pos}\n";
    }

    #LSU
    for my $pos (sort {$a <=> $b} keys %{$transcript_LSU_5prime_counts_by_positon{$gene}}){
         print OUT6 "$gene,LSU,$pos,$transcript_LSU_5prime_counts_by_positon{$gene}{$pos}\n";
    }
}
close(OUT6);
