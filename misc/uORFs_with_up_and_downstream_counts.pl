    ...
    if (exists ($gene_to_uORF{$gene})){ 
        ...
        for my $uORF_id (sort keys %{ $gene_to_uORF{$gene} }){
            ...
            if (exists ($uORF_start_coord{$gene}{$uORF_id})){
                ...
                $uORF_TSS_TIS_SSU_coverage_by_positon{$uORF_id}{"uORF_start_to_tis"}=0; #initialise
                $uORF_TSS_TIS_LSU_coverage_by_positon{$uORF_id}{"uORF_start_to_tis"}=0; #initialise

                for my $coord ($uORF_start_coord .. ($gene_start_coord-1)){
                    my $pos=$gene_model_fwd{$gene}{$coord};
                    if (exists ($uORF_TSS_TIS_window_search_fwd{$chr}{$pos})){
                        $uORF_TSS_TIS_window_search_fwd{$chr}{$pos}.="-".$uORF_id."~uORF_start_to_tis";
                    }else{
                        $uORF_TSS_TIS_window_search_fwd{$chr}{$pos}=$uORF_id."~uORF_start_to_tis";                    
                    }
                }      
        ...
        for my $uORF_id (sort keys %{ $gene_to_uORF{$gene} }){
            if (exists ($uORF_start_coord{$gene}{$uORF_id})){
                ...
                for my $coord ($uORF_start_coord .. ($gene_start_coord-1)){
                    my $pos=$gene_model_rev{$gene}{$coord};
                    if (exists ($uORF_TSS_TIS_window_search_rev{$chr}{$pos})){
                        $uORF_TSS_TIS_window_search_rev{$chr}{$pos}.="-".$uORF_id."~uORF_start_to_tis";
                    }else{
                        $uORF_TSS_TIS_window_search_rev{$chr}{$pos}=$uORF_id."~uORF_start_to_tis";
                    }
                }
                    ...
                    if ($cigar =~ /^([0-9]+[MIDN])/){
                        ...
                        if ($cigar_part =~ /(\d+)M/){   #alignment matching 
                            for my $pos ($leftMost .. ($leftMost+$1-1)){ #search though this position
                                ...
                                if (exists ($uORF_TSS_TIS_window_search_rev{$chr}{$pos})){
           #                         print "position line=$uORF_TSS_TIS_window_search_rev{$chr}{$pos}\n";
...
for my $gene (keys %gene_model_fwd){
    ...
    if (exists ($gene_to_uORF{$gene})){
        ...
        for my $uORF (keys %{ $gene_to_uORF{$gene} } ) {
            ...
            if (exists ($uORF_TSS_TIS_SSU_coverage_by_positon{$uORF}{"uORF_start_to_tis"})){
                $u_SSU_down=$uORF_TSS_TIS_SSU_coverage_by_positon{$uORF}{"uORF_start_to_tis"};
            }
            #}else{ print "bad_uORF,fwd,SSU,to_TIS,$uORF,$gene\n"; }

            if (exists ($uORF_TSS_TIS_LSU_coverage_by_positon{$uORF}{"uORF_start_to_tis"})){
                $u_LSU_down=$uORF_TSS_TIS_LSU_coverage_by_positon{$uORF}{"uORF_start_to_tis"};
            }
            #}else{ print "bad_uORF,fwd,LSU,to_TIS,$uORF,$gene\n"; }

            print OUT1 "$gene_info,$u_kozak,$u_length,$u_up_distance,$u_down_distance,$u_stop_to_TIS,$u_start,$u_stop,$u_SSU_up,$u_LSU_up,$u_SSU_down,$u_LSU_down\n"; 
        }
    }
}
...
for my $gene (keys %gene_model_rev){
    ...
    if (exists ($gene_to_uORF{$gene})){
        ...
        for my $uORF (keys %{ $gene_to_uORF{$gene} } ) {
            ...
            #my $u_SSU_down=$uORF_TSS_TIS_SSU_coverage_by_positon{$uORF}{"uORF_start_to_tis"};
            #my $u_LSU_down=$uORF_TSS_TIS_LSU_coverage_by_positon{$uORF}{"uORF_start_to_tis"};
